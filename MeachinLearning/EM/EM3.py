...
if __name__ == '__main__':
    ...
    for i, type in enumerate(types):
        ...
        bic[i] = gmm.bic(x)
    print('error rate: {}'.format(err.ravel()))
    print('BIC:{}'.format(bic.ravel()))
    ...
    plt.legend([b1[0],b2[0]],('errorRate', 'BIC'))
    plt.title('error rate and BIC on different cov', fontsize=18)
    #plt.show()

    optimal = bic.argmin() ###获取BIC 最小的cov type 的index 然后创建一个混合高斯分布，得到预测的各个高斯分布的均值和方差
    gmm = GaussianMixture(n_components=2, covariance_type=types[optimal], random_state=0) 
    gmm.fit(x)
    print('average = \n{}'.format(gmm.means_))
    print('covariance = \n {}'.format(gmm.covariances_))
    y_hat = gmm.predict(x) ##此处用训练好的模型 在对x进行预测 得到y_hat
    ...
    if gmm.means_[0][0] > gmm.means_[1][0]:  ####第一项是y=0, 第二项y=1
        z = grid_hat ==0 
        grid_hat[z] = 1
        grid_hat[~z] = 0
    ...
    plt.title('GMM parameter: covariance_type=%s'%types[optimal], fontsize=20)
    plt.grid()
    plt.show()